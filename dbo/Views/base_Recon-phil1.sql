...
CREATE view [dbo].[base_Recon-phil1]
 as 
	SELECT 
		...
		Max([dbo].[map_order_2023].[Desk]) AS Desk, 
		Max([dbo].[map_order_2023].[SubDesk]) AS SubDesk,
		Max([dbo].[map_order_2023].[SubDesk]) AS RevRecSubDesk,
...
FROM 
	...
	LEFT JOIN [dbo].[map_order_2023] ON [dbo].[Recon_zw1_Archive-phil].[OrderNo] = [dbo].[map_order_2023].[OrderNo]
WHERE 
	(
