...
CREATE view [dbo].[AWV_Results_Diff] as select distinct dd.[CompanyCode] as [SAP-Buchungskreis], dd.[Account] as [SAP-Konto],dd.[OffsettingAccount] as [SAP-Debitor/Kreditor],dd.[DocumentNumber] as [SAP-Belegnummer], dd.[PostingKey] as [SAP-Buchungsschlüssel],dd.[DocumentType] as [SAP-Belegart],
dd.DocumentHeaderText as [SAP-Belegkopftext],dd.[PostingDate] as [SAP-Buchungsdatum],dd.[Reference] as [SAP-Referenz],dd.[Text] as [SAP-Text],dd.[DocumentDate] as [SAP-Belegdatum],dd.Assignment as [SAP-Zuordnung],dd.[RefKey3] as [RefSchl2],
...
where dd.[CompanyCode] = (600) and gg.[AWV-Responsible] is NOT NULL and ww.[SAPAccount] = dd.[Account] and ww.[SAPDocumentNumber] = dd.[DocumentNumber] and gg.[AWV-Required] <> 'Nein' and gg.[AWV-Required] = 'Ja'
