...
/*changes: 2022-06-10: added 'RWE RENEWABLES' */

CREATE view [dbo].[Recon_Diff_GloRi] as 
	SELECT 
		InternalLegalEntity, 
		Desk, 
		Subdesk, 
		...
		case when recon.recongroup = 'Exchanges' then 
			case when recon.account like '4%' then 
				case when recon.InternalLegalEntity = 'RWEST DE' then 'N5' else case when recon.InternalLegalEntity = 'RWEST UK'  then '28' else 'AN' end end
			else case when recon.InternalLegalEntity = 'RWEST DE' then 'VM' else case when recon.InternalLegalEntity = 'RWEST UK'  then '88' else 'VN' end end end else 
		...
		case when [ReconGroup]='Physical Power' 
						and (d.[ExtLegalEntity] not In ('RWEST CZ PE','RWEST DE - PE','RWEST UK - PE','RWEST ASIA PACIFIC PE','RWEST PARTICIPATIONS PE','RWE TS DE PE','RWE TS UK PE','RWE POWER','RWE GENERATION LE','INNOGY SPAIN','RWE WINDPOWER NL','RWE RENEWABLES','Amrum-Offshore West GmbH') or d.[ExtLegalEntity] is null)
	  					and Month((select [asofdate_eom] from dbo.asofdate)) In (3,6,9,12)
			THEN Replace([account],'04','01') 	ELSE [account] end AS [Konto_GUV], 


		case when ([internallegalentity] not in ('RWEST CZ') and (d.[AccrualOnDebitor] = 1) and ((d.ctpygroup = 'External' or (d.[ExtLegalEntity] In ('RWEST CZ PE','RWEST DE - PE','RWEST UK - PE','RWEST ASIA PACIFIC PE','RWEST PARTICIPATIONS PE','RWE TS DE PE','RWE TS UK PE','RWE POWER','RWE GENERATION LE','INNOGY SPAIN','RWE WINDPOWER NL','RWE RENEWABLES','WINDPARK NORDSEE OST','RWE RENEWABLES ITALIA','Amrum-Offshore West GmbH'))) or (Month((select [asofdate_eom] from dbo.asofdate)) not In (3,6,9,12))))
		...
		else case when ((d.[AccrualOnDebitor] = 1) and ((d.ctpygroup = 'External' or (d.[ExtLegalEntity] In ('RWEST CZ PE','RWEST DE - PE','RWEST UK - PE','RWEST ASIA PACIFIC PE','RWEST PARTICIPATIONS PE','RWE TS DE PE','RWE TS UK PE','RWE POWER','RWE GENERATION LE','INNOGY SPAIN','RWE WINDPOWER NL','RWE RENEWABLES','WINDPARK NORDSEE OST','RWE RENEWABLES ITALIA','Amrum-Offshore West GmbH'))) or (Month((select [asofdate_eom] from dbo.asofdate)) not In (3,6,9,12))))
			THEN d.Debitor 
	...
	from 
		((dbo.recon 
			LEFT JOIN (SELECT OrderNo, Max(Ref3) AS Ref3 FROM dbo.map_order GROUP BY OrderNo) as r ON dbo.recon.orderno = r.orderno)
			LEFT JOin (SELECT partner, max(ctpygroup) as ctpygroup from dbo.map_counterparty group by partner) as c on dbo.recon.partner = c.partner)
			LEFT JOIN dbo.map_counterparty as d ON dbo.recon.externalbusinessunit = d.extbunit
	WHERE 
		ReconGroup not in ('prüfen', 'MTM', 'not relevant') 
		AND ReconGroup not like 'non-IAS%' AND
		(
			(
				(
						 abs([Diff_Volume])>1 
