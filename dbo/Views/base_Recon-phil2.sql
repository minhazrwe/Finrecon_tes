...
CREATE view [dbo].[base_Recon-phil2]
 as 
	SELECT 
		...
		Max([dbo].[00_map_order_2023].[Desk]) AS Desk, 
		Max([dbo].[00_map_order_2023].[SubDesk]) AS SubDesk,
		Max([dbo].[00_map_order_2023].[SubDesk]) AS RevRecSubDesk,
...
FROM 
	...
	LEFT JOIN [dbo].[00_map_order_2023] ON [dbo].[Recon_zw1_Archive-phil].[OrderNo] = [dbo].[00_map_order_2023].OrderNo
WHERE 
	(
