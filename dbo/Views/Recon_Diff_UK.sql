...
CREATE view [dbo].[Recon_Diff_UK] as 
SELECT InternalLegalEntity
	,Desk
	,IIF(RevRecSubdesk = '',SubDesk,RevRecSubdesk) as SubDesk
	,ReconGroup
	,[dbo].[Recon].[OrderNo]
	,DeliveryMonth
	,DealID_Recon
	...
	,DocumentNumber
	,Reference
	,CASE WHEN dbo.Recon.Partner IS NULL or dbo.Recon.Partner = 'NULL' THEN '/' ELSE dbo.Recon.Partner END AS partner
