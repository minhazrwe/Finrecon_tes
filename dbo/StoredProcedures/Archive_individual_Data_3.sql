	...
	CREATE PROCEDURE [dbo].[Archive_individual_Data_3] 
	AS
	BEGIN TRY
	...
	select @sql = 'SELECT *  FROM [FinRecon].[dbo].[Archive_individual_data_sql_3]'

	Select @step = @step +1
	BEGIN insert into [dbo].[Logfile] select @step, GETDATE () END

	select @export_path = @path + 'FASTracker_Archive_2023_03_06_09--' + @Time_of_archiving + '.txt' 
