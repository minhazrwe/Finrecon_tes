...
-----------------------------------------------------------------------------------------------------------------
	...
	2024-03-26, PG,			step 30, added "GPM Performance UK" to Recon_diff_tbl
    2024-07-16 MT           STEP 33" added RWETA PE in where clause of Recon_UK_ALL . Recon_UK_Adjustments and Recon_UK_Diff table creation process. (reason: export recon file for AMERICAS TRADING AND ORIGINATION DESK)
=================================================================================================================*/


CREATE PROCEDURE [dbo].[Reconciliation]
...
BEGIN TRY
		...
		( 
			(
				...
				OR user_name () ='ENERGY\R920983'			/*April Xin */ --delete after 10.05.2024
			...
			(
				(
					...
					OR user_name () ='ENERGY\UI710222'	/*Dennis Schley*/
					OR user_name () = 'ENERGY\R884018'  /*VP*/
					OR user_name () = 'ENERGY\UI908266' -- Geoff O-A
				) 
		...
		BEGIN
			select @step = 10
			-- 2024-04-23 MK: Execution of InsertIntoRecon looks redundant (is done ~30 lines below as well), but is necessary, since UpdateEndurAccounts uses data from Recon table (recursive work).
    ...
    (
		...
		where 
			...
			/* include GPM Perfomance UK in Recon_Diff_tbl for Diff Comm Gas - Request from April Xin - PG 26.03.2024 */
			OR Subdesk = 'GPM Performance UK'
		...
		SELECT [dbo].[00_map_order].LegalEntity
			...
			,[Adjustments].OrderNo
			,[Adjustments].Internal_Portfolio_ID as PortfolioID
		...
		WHERE (
                ...
                OR [dbo].[00_map_order].LegalEntity = 'RWETA PE'
				)
