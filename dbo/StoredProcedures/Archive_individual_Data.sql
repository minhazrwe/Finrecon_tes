	...
	CREATE PROCEDURE [dbo].[Archive_individual_Data] 
	AS
	BEGIN TRY
	...
	select @sql = 'SELECT *  FROM [FinRecon].[dbo].[Archive_individual_data_sql]'

	Select @step = @step +1
	BEGIN insert into [dbo].[Logfile] select @sql, GETDATE () END

	select @export_path = @path + 'FASTracker_Archive_2023-07-08-10-11--' + @Time_of_archiving + '.txt' 

	Select @step = @step +1
	--BEGIN insert into [dbo].[Logfile] select  @export_path + ' Anfang' , GETDATE () END

	exec master.filesystem.CsvExport @sql,  @export_path , TRUE, TRUE, '~'

	Select @step = @step +1
	--BEGIN insert into [dbo].[Logfile] select @export_path + ' Ende', GETDATE () END
END TRY
