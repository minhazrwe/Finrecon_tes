...
-- Author:      MKB			
-- Created:     07/2022
-- Description:	importing data from csv-file "rwest_olf_numbers"
-- =============================================
*/

---drop PROCEDURE dbo.Import_KIDFX_OLF_NUMBERS
CREATE PROCEDURE [dbo].[Import_KIDFX_OLF_NUMBERS] 
AS
BEGIN TRY
  ...
  DECLARE @step Integer		
	...
	SET @FileSource = 'KIDFX_OLF_NUMBERS'
  ...
  --/* get Info if Logging is enabled */
	...
	SELECT @step=20
	IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'dbo' AND TABLE_NAME = 'table_KIDFX_RWEST_OLF_NUMBERS_tmp'))
	BEGIN DROP TABLE dbo.table_KIDFX_RWEST_OLF_NUMBERS_tmp END
	
	SELECT @step=25
	CREATE TABLE dbo.table_KIDFX_RWEST_OLF_NUMBERS_tmp
	(
	...
	WHILE @counter >0
      ...
      WHERE ROW = @counter
			...
			SELECT @sql = N'BULK INSERT [dbo].[table_KIDFX_RWEST_OLF_NUMBERS_tmp] FROM '  + '''' + @PathName + @filename + ''''  + ' WITH (CODEPAGE = 1252, FIELDTERMINATOR =''~'', FIRSTROW = 2, ROWTERMINATOR =''0x0a'')';
			--SELECT @sql = N'BULK INSERT [dbo].[table_KIDFX_RWEST_OLF_NUMBERS_tmp] FROM '  + '''' + @PathName + @filename + ''''  + ' WITH (CODEPAGE = 1252, FIELDTERMINATOR =''~'', FIRSTROW = 2, ROWTERMINATOR =''\n'')';
...
/********************************************************************************************************************************************************/
		...
		SELECT @step=70
		truncate TABLE dbo.table_KIDFX_KID_ID_OLF
		
		SELECT @step=75
	INSERT INTO [dbo].[table_KIDFX_KID_ID_OLF]
		(
			[Portfolio_Name]
			,[WSS_Number]
			,[OLF_Number]
			,[OLF_Number_Transaction_Comments]
			,[Instrument]
			,[Param9_System_Reference]
			,[CP]
			,[UTI]
			,[Parent_Deal_Number]
		)
      ...
      ,[Parent_Deal_Number]
			FROM 
				dbo.table_KIDFX_RWEST_OLF_NUMBERS_tmp
